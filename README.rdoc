...
/ Make sure Users#show is responsive
/ Make sure Users#New is responsive
/ Make sure step1 and step2 image for Users#New is good 

Make sure CommonApp is more easily filled in. i.e default for numbers with select is not 0

Make it possible to see all applicants for a job with admin

Figure out password reset password validations

Intro to community consulting 

Intro to frontiers book 

On Job, user can not be associated twice 
...
For setting up customerio with rails -- 

heroku config:set CIO_SITE_ID= CIO_API_KEY=


----

For later, if want to use sunspot rails 
  In users#index 
  ...
  Things that must be seeded :

  Positions, cities, industries, heats 
